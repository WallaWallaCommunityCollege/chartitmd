...
// Expose support vars for convenience
	...
	---------------------------------------------------------------------- */
                    ...
                    contains = hasCompare || rnative.test(docElem.contains) ? function (a, b) {
                        ...
                        return a === bup || !!(bup && bup.nodeType === 1 && (adown.contains ? adown.contains(bup) : a.compareDocumentPosition && a.compareDocumentPosition(
                                    bup) & 16));
                    } : function (a, b) {
                        if (b) {
	...
	---------------------------------------------------------------------- */
                ...
                Sizzle.matchesSelector = function (elem, expr) {
                    ...
                    if (support.matchesSelector && documentIsHTML && !compilerCache[expr + " "] && (!rbuggyMatches || !rbuggyMatches.test(
			...
			*/
                        ...
                        "TAG": function (nodeNameSelector) {
                            ...
                            return function (elem) {
                                ...
                                result += "";
                                return operator === "=" ? result === check : operator === "!=" ? result !== check : operator === "^=" ? check && result.indexOf(
                                    check) === 0 : operator === "*=" ? check && result.indexOf(check) > -1 : operator === "$=" ? check && result.slice(
                                    -check.length) === check : operator === "~=" ? (" " + result.replace(rwhitespace,
                                                                                                         " ") + " ").indexOf(
                                    check) > -1 : operator === "|=" ? result === check || result.slice(0,
                ...
                function matcherFromTokens(tokens) {
                    var checkContext, matcher, j, len = tokens.length, leadingRelative = Expr.relative[tokens[0].type],
                        ...
                        matchContext = addCombinator(function (elem) {
                            ...
                            var ret = (!leadingRelative && (xml || context !== outermostContext)) || ((checkContext = context).nodeType ? matchContext(
                                    elem,
                                    context,
    ...
    jQuery.extend({
                      Deferred: function (func) {
                          var tuples = [// action, add listener, callbacks,
                                  ...
                                  ["notify", "progress", jQuery.Callbacks("memory"), jQuery.Callbacks("memory"), 2], ["resolve", "done", jQuery.Callbacks(
                                  "once memory"), jQuery.Callbacks("once memory"), 0, "resolved"], ["reject", "fail", jQuery.Callbacks(
                                  "once memory"), jQuery.Callbacks("once memory"), 1, "rejected"]], state = "pending",
                              promise = {
                                  state: function () {
                                      return state;
                                  }, always: function () {
                        ...
                        for (i = 0; i < delegateCount; i++) {
                            ...
                            if (matchedSelectors[sel] === undefined) {
                                matchedSelectors[sel] = handleObj.needsContext ? jQuery(sel, this)
                                    .index(cur) > -1 : jQuery.find(sel, this, null, [cur]).length;
                            }
...
// a vendor prefixed property.
    ...
    function getWidthOrHeight(elem, dimension, extra) {
        ...
        return (val + boxModelAdjustment(elem,
                                         dimension,
        ...
        jQuery.cssHooks[dimension] = {
            get: function (elem, computed, extra) {
                ...
                if (isBorderBox && support.scrollboxSize() === styles.position) {
                    subtract -= Math.ceil(elem["offset" + dimension[0].toUpperCase() + dimension.slice(1)] - parseFloat(
                        styles[dimension]) - boxModelAdjustment(elem, dimension, "border", false, styles) - 0.5);
                }
...
// Generate parameters to create a standard animation
    ...
    function Animation(elem, properties, options) {
        var result, stopped, index = 0, length = Animation.prefilters.length, deferred = jQuery.Deferred()
                                                                                               .always(function () {
                              ...
                              get: function (elem) {
                                  ...
                                  for (; i < max; i++) {
                                      ...
                                      if ((option.selected || i === index) && // Don't return options that are disabled or in a disabled optgroup
                                          !option.disabled && (!option.parentNode.disabled || !nodeName(option.parentNode,
                                                                                                        "optgroup"))) {
                                  ...
                                  while (i--) {
                                      ...
                                      if (option.selected = jQuery.inArray(jQuery.valHooks.option.get(option),
                                                                           values) > -1) {
                                          optionSet = true;
                                      }
                ...
                return elements ? jQuery.makeArray(elements) : this;
                       ...
                       .filter(function () {
                           ...
                           return this.name && !jQuery(this)
                               .is(":disabled") && rsubmittable.test(this.nodeName) && !rsubmitterTypes.test(type) && (this.checked || !rcheckableType.test(
                                   type));
                       })
                       .map(function (i, elem) {
                           var val = jQuery(this)
		...
		*/
                      ...
                      ajax: function (url, options) {
                          ...
                          var transport, // URL without anti-cache param
                              ...
                              globalEventContext = s.context && (callbackContext.nodeType || callbackContext.jquery) ? jQuery(
                                  callbackContext) : jQuery.event, // Deferreds
                          ...
                          } else if (s.data && s.processData && (s.contentType || "").indexOf(
...
// Attach a bunch of functions for handling common AJAX events
    ...
    jQuery.offset = {
        setOffset: function (elem, options, i) {
            ...
            calculatePosition = (position === "absolute" || position === "fixed") && (curCSSTop + curCSSLeft).indexOf(
                "auto") > -1;
    ...
    jQuery.fn.extend({
                         ...
                         position: function () {
                             ...
                             if (jQuery.css(elem, "position") === "fixed") {
                                 ...
                                 while (offsetParent && (offsetParent === doc.body || offsetParent === doc.documentElement) && jQuery.css(
                                     offsetParent,
                                     "position") === "static") {
